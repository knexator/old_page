...
/*let taco_sprite: any;
let taco_texture = new PintarJS.Texture('taco.png', () => {
  // for (let k=0; k<16; k++) {
  taco_sprite = new PintarJS.Sprite(taco_texture)
...
class TacoShader extends PintarJS.ShaderBase {
    ...
    get vertexShaderCode() {
      ...
      void main()
      {
        ...
        vec2 res = delta * coords.x + perp * coords.y;
        ...
        v_texCoord = vec2(a_position.x - .5, a_position.y * length(delta));
      }
      `;
    }
    /**
    ...
    void main()
    {
      float d = max(abs(v_texCoord.x), (0.5 - v_texCoord.y) * 0.85);
      d = smoothstep(0.40, 0.41, d);
      // float d = min(v_texCoord.x, min(1.0 - v_texCoord.x, v_texCoord.y));
      // float v = smoothstep(0.1, 0.2, d);
      gl_FragColor = vec4(vec3(d * .2), d);
    ...
    if (isButtonDown("left") && last_pressed) {
        ...
        pintar.drawLine(
          new PintarJS.ColoredLine(
            new PintarJS.Point(last_pressed.x, last_pressed.y),
            new PintarJS.Point(mouse.x, mouse.y),
            PintarJS.Color.red()
          )
        );*/
        cur_taco_tail = [
