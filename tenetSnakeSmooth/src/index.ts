...
function update(curTime: number) {
  ...
  if (isKeyDown("d") || isKeyDown("a")) {
    let delta_angle = ((isKeyDown("d") ? 1 : 0) - (isKeyDown("a") ? 1 : 0)) * (TURN_SPEED * deltaTime)
...
function touching(new_head: { i: number; j: number }, cur_apple: { i: number; j: number; }) {
