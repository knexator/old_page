...
function getId(thing: Thing): string {
    return JSON.stringify(thing);
}

function isSmallerThan(a: Thing, b: Thing) {
    if (a === b) return false;
...
let isSmallerThan_lut = new ManyKeysMap<[Thing, Thing], boolean>();
let draw_edge = new ManyKeysMap<[Thing, Thing], boolean>();
...
for (const small of elements) {
    for (const big of elements) {
        if (!isSmallerThan_lut.get([small, big])) continue;
        for (const bigger of elements) {
            if (!isSmallerThan_lut.get([big, bigger])) continue;
