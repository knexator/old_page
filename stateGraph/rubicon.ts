...
// actual logic
// { GameState, initialState, nextStates, stateId, isWon, stateToPos }
...
export function nextStates(state: GameState): Record<string, GameState> {
    let result: Record<string, GameState> = {
        "empty": {
            player: !state.player,
...
function validState(state: GameState): boolean {
    // is cabbage safe?
...
export function stateId(state: GameState) {
    return `${state.player},${state.cabbage},${state.sheep},${state.wolf}`;
}

export function stateToPos(state: GameState) {
    return {
...
function expandNode(cur_node) {
    ...
    for (const [input, new_state] of Object.entries(next_states)) {
        ...
        if (nodes.get(new_id) === null) {
            addNode(new_state as GameState, new_id, cur_node);
            // nodes.add({
            //     id: new_id,
            //     state: new_state,
            //     won: new_state.isWon(),
...
function addNode(state: GameState, id: string | undefined = undefined, parent_node: Node | undefined = undefined) {
    if (id === undefined) id = stateId(state);
