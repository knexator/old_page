...
// 2 = red
const n_colors = 3;
const size: Vec = { x: 5, y: 5 };
const walls_vertical: number[][] = [
    [-1, 2, 1, -1],
    [1, 2, 2, 1],
    [0, 2, 0, 1],
    [0, 1, 0, 1],
...
export function nextStates(state: GameState): Record<string, GameState> {
    ...
    return result as Record<string, GameState>;
}

function nextState(state: GameState, dir: Vec): GameState | false {
    let new_player_pos = {
