...
let balls_vel = [];
let balls_won = [];
let target_balls_pos = [];
let target_balls_vel = [];
for (let k = 0; k < N_BALLS; k++) {
    balls_pos.push(new Float32Array(N_WORLDS * 2));
    balls_vel.push(new Float32Array(N_WORLDS * 2).fill(0.0));
    target_balls_pos.push(new Float32Array(N_WORLDS * 2));
    target_balls_vel.push(new Float32Array(N_WORLDS * 2).fill(0.0));
...
function collapse_White() {
    ...
    for (let k = 0; k < N_WORLDS * 2; k += 2) {
        if (balls_won[n_b][k / 2] === 0) {
            ...
            if (!example_won)
                example_won = [balls_pos[n_b][k], balls_pos[n_b][k + 1]];
            n_won += 1;
        }
    }
    mean_px /= N_WORLDS;
...
function collapse_noWhite() {
    ...
    mean_vy /= N_WORLDS;
    if (n_lost > n_won) {
        let cur_ball_pos = target_balls_pos[n_b];
        let cur_ball_vel = target_balls_vel[n_b];
        for (let k = 0; k < N_WORLDS * 2; k += 2) {
            cur_ball_pos[k] = mean_px;
            cur_ball_pos[k + 1] = mean_py;
            cur_ball_vel[k] = mean_vx;
    ...
    for (let k = 0; k < N_WORLDS * 2; k += 2) {
        target_balls_pos[0][k] += Math.cos(Math.PI * k / N_WORLDS) * CHAOS_AMOUNT;
        target_balls_pos[0][k + 1] += Math.sin(Math.PI * k / N_WORLDS) * CHAOS_AMOUNT;
    }
}
...
class BallShader extends PintarJS.ShaderBase {
    ...
    prepare(_renderable, _viewport) {
        ...
        this._gl.uniform2f(this.uniforms.u_pos, balls_pos[cur_selected[1]][cur_selected[0] * 2], balls_pos[cur_selected[1]][cur_selected[0] * 2 + 1]);
...
class TacoShader extends PintarJS.ShaderBase {
    ...
    prepare(_renderable, _viewport) {
        this._gl.uniform2f(this.uniforms.u_head, cur_taco_head[0], cur_taco_head[1]);
        this._gl.uniform2f(this.uniforms.u_tail, cur_taco_tail[0], cur_taco_tail[1]);
    }
}
...
function update(cur_time) {
    ...
    if (collapseT > 0) {
        for (let n = 0; n < N_BALLS; n++) {
            for (let k = 0; k < N_WORLDS * 2; k++) {
                if (target_balls_pos[n][k] !== 0) {
                    balls_pos[n][k] = lerp(balls_pos[n][k], target_balls_pos[n][k], 0.2);
                    balls_vel[n][k] = lerp(balls_vel[n][k], target_balls_vel[n][k], 0.2);
                }
            }
        }
        collapseT = Math.max(0, collapseT - deltaTime * 0.001);
    }
    else {
...
pintar.canvas.addEventListener('mouseleave', (_e) => {
    mouse.active = false;
});
pintar.canvas.addEventListener('mouseenter', (_e) => {
    mouse.active = true;
});
