...
function zeroDelays(n_mod: number) {
...
function selfConnections(n_mod: number) {
  let res: [[number, number], [number, number], [number, number]][] = [];
  for (let k=0; k<n_mod; k++) {
...
function emptyData(n_mod: number, max_t: number) {
  let res: [(boolean | null | undefined)[], (boolean | null | undefined)[], (boolean | null | undefined)[]][] = [];
  for (let k=0; k<n_mod; k++) {
    res.push([Array(max_t).fill(undefined), Array(max_t).fill(undefined), Array(max_t).fill(undefined)]);
  }
  return res;
}
...
function findFaultyConnections() {
  let res: [boolean, boolean, boolean][] = [];
...
function findReverseConnections() {
  ...
  for (let source_mod=0; source_mod<N_MODULES; source_mod++) {
    for (let source_part=0; source_part<3; source_part++) {
      if (faultyConnections[source_mod][source_part]) continue;
      let [target_mod, target_part] = connections[source_mod][source_part];
...
function update(curTime: number) {
  ...
  let mi = Math.floor((mouse.x + table_off_x) / BUTTON_W) - 1;
  let mj = Math.floor((mouse.y + table_off_y) / BUTTON_H) - 1;

  if (!EDITOR) {
    if (isKeyDown('a')) table_off_x -= deltaTime * 1.0;
    if (isKeyDown('d')) table_off_x += deltaTime * 1.0;
  }
  if (isKeyDown('w')) table_off_y -= deltaTime * 1.0;
  if (isKeyDown('s')) table_off_y += deltaTime * 1.0;
  ...
  if (!EDITOR) {
    for (let i=min_i; i <= max_i; i++) {
      for (let j=min_j; j<= max_j; j++) {
        ...
        let text = (data_val === null) ? "?" : (data_val === undefined) ? "-" : data_val ? "✓" : "✗";
    ...
    if (mi > 0 && mi + 1 < MAX_T && mj >= 0 && mj < N_MODULES * 3) {
      ...
      let m_part = mj % 3;
      if (wasButtonPressed("left") || wasButtonPressed("right")) {
        data[m_mod][m_part][mi] = cycleInput(data[m_mod][m_part][mi], wasButtonPressed("left"));
        writeContradictions();
      }
    ...
    if (mj >= 0 && mj < 3 * N_MODULES) {
      ...
      let m_mod = Math.floor(mj / 3);
      let m_part = mj % 3;
      mi = Math.floor(mouse.x / BUTTON_W) - 1;
      if (mi === 1) {
        let [target_mod, target_part] = connections[m_mod][m_part];
        let [original_target_mod, original_target_part] = connections[m_mod][m_part];
        if (wasKeyPressed('a')) {
          target_part = 0;
        } else if (wasKeyPressed('b')) {
          target_part = 1;
        } else if (wasKeyPressed('c')) {
          target_part = 2;
        }
        for (let k=1; k<=9; k++) {
          if (k <= N_MODULES && wasKeyPressed(k.toString())) {
            target_mod = k - 1;
          }
        }
        let target_j = target_mod * 3 + target_part;
        target_j = mod(target_j - mouse.wheel, N_MODULES * 3);
        ...
        delays[m_mod][m_part] -= mouse.wheel;
      } else {
        document.body.style.cursor = 'default';
      }
    } else {
...
function cycleInput(cur: boolean | null | undefined, left: boolean): boolean | null | undefined {
  if (left) {
    if (cur === true) return undefined;
    return true;
  } else {
    if (cur === false) return undefined;
    return false;
  }
}
