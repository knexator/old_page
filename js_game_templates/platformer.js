...
class Player {
    ...
    wouldOverlapAfterMove(pos, dx, dy) {
        let margin = 0.01;
...
Player.WALK_ACC = 2.0; // perfect
Player.JUMP_SPEED = 22.0;
Player.GRAVITY = 50.0;
Player.VERTICAL_FRICTION = 0.02;
Player.HORIZONTAL_FRICTION = 0.3; // perfect
class Level {
    constructor(strData) {
        // level data
    ...
    overlaps(rect) {
        ...
        if (i1 < 0 || j1 < 0 || i2 >= this.w || j2 >= this.h)
            return true;
        for (let i = i1; i <= i2; i++) {
            for (let j = j1; j <= j2; j++) {
                if (this.geo[j][i] == 1)
